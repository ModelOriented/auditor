...
#'
#' @export
plotModelRanking <- function(object, ..., scores = c("MAE", "MSE", "REC", "RROC"), new.score = NULL) {

  # safeguard
  x <- y <- value <- scaled <- name <- label <- NULL
  # name <- score <- label <- x2 <- y2 <- label2 <- NULL

  # check if passed object is of class "modelPerformance" or "modelAudit"
  check_object(object, type = "prfm")

  # data frame for ggplot object
  df <- make_dataframe(object, ..., scores = scores, new.score = new.score, type = "prfm")
